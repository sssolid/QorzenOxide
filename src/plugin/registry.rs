...
#[derive(Debug, Default)]
pub struct ExamplePluginWrapper {
    inner: Option<Box<dyn Plugin>>,
}

#[cfg(feature = "example_plugin")]
#[async_trait]
impl Plugin for ExamplePluginWrapper {
    fn info(&self) -> PluginInfo {
        PluginInfo {
    ...
    async fn initialize(&mut self, _context: crate::plugin::PluginContext) -> Result<()> {
        ...
        // In a real implementation, we would create the actual plugin instance here
    ...
    fn ui_components(&self) -> Vec<crate::plugin::UIComponent> {
        vec![
            crate::plugin::UIComponent {
                id: "product_list".to_string(),
                name: "Product List".to_string(),
                component_type: crate::plugin::ComponentType::Page,
                props: serde_json::json!({
                    "title": "Products",
                    "searchable": true
                }),
    ...
    fn menu_items(&self) -> Vec<crate::plugin::MenuItem> {
        vec![
    ...
    fn settings_schema(&self) -> Option<crate::config::SettingsSchema> {
        Some(crate::config::SettingsSchema {
            version: "1.0".to_string(),
            schema: serde_json::json!({
                "type": "object",
                "properties": {
    ...
    fn api_routes(&self) -> Vec<crate::plugin::ApiRoute> {
        vec![
            crate::plugin::ApiRoute {
                ...
                rate_limit: Some(crate::plugin::RateLimit {
                    requests_per_minute: 60,
                    burst_limit: 10,
                }),
    ...
    fn render_component(&self, component_id: &str, _props: serde_json::Value) -> Result<dioxus::prelude::VNode> {
        ...
        match component_id {
            "product_list" => {
                Ok(VNode::from(rsx! {
                    div { class: "product-catalog-plugin p-6",
                        div { class: "mb-6",
                            h2 { class: "text-2xl font-bold text-gray-900 mb-2", "Product Catalog" }
                            p { class: "text-gray-600", "Manage your product inventory and catalog." }
                        }
                        ...
                        div { class: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6",
                            ...
                            div { class: "bg-white rounded-lg shadow p-4 border",
                                div { class: "flex items-center mb-3",
                                    ...
                                    div {
                                        h3 { class: "font-semibold text-gray-900", "iPhone 15 Pro" }
                                        p { class: "text-sm text-gray-500", "Electronics" }
                                    }
                                }
                                p { class: "text-gray-600 text-sm mb-3", "Latest iPhone with advanced camera system" }
                                div { class: "flex justify-between items-center",
                                    span { class: "text-lg font-bold text-blue-600", "$999.00" }
                                    span { class: "text-sm text-green-600", "In Stock: 25" }
                                }
                            }
                            ...
                            div { class: "bg-white rounded-lg shadow p-4 border",
                                div { class: "flex items-center mb-3",
                                    span { class: "text-2xl mr-3", "💻" }
                                    div {
                                        h3 { class: "font-semibold text-gray-900", "MacBook Pro" }
                                        p { class: "text-sm text-gray-500", "Computers" }
                                    }
                                }
                                p { class: "text-gray-600 text-sm mb-3", "Professional laptop with M3 chip" }
                                div { class: "flex justify-between items-center",
                                    span { class: "text-lg font-bold text-blue-600", "$1,999.00" }
                                    span { class: "text-sm text-green-600", "In Stock: 12" }
                                }
                            }
                            ...
                            div { class: "bg-white rounded-lg shadow p-4 border",
                                div { class: "flex items-center mb-3",
                                    span { class: "text-2xl mr-3", "⌚" }
                                    div {
                                        h3 { class: "font-semibold text-gray-900", "Apple Watch" }
                                        p { class: "text-sm text-gray-500", "Wearables" }
                                    }
                                }
                                p { class: "text-gray-600 text-sm mb-3", "Smart watch with health monitoring" }
                                div { class: "flex justify-between items-center",
                                    span { class: "text-lg font-bold text-blue-600", "$399.00" }
                                    span { class: "text-sm text-orange-600", "Low Stock: 3" }
                                }
                            }
                        }
                        ...
                        div { class: "mt-8 p-4 bg-blue-50 rounded-lg",
                            div { class: "flex items-center",
                                ...
                                div {
                                    h4 { class: "font-semibold text-blue-900", "Plugin Status" }
                                    p { class: "text-blue-700 text-sm", "Product Catalog plugin is running and ready to use." }
                                }
                            }
                        }
                    }
    ...
    async fn handle_api_request(&self, route_id: &str, _request: crate::plugin::ApiRequest) -> Result<crate::plugin::ApiResponse> {
        match route_id {
            "list_products" => {
                let sample_products = serde_json::json!([
                    {
                        "id": "prod_001",
                        "name": "iPhone 15 Pro",
                ...
                Ok(crate::plugin::ApiResponse {
                    status_code: 200,
                    description: "Success".to_string(),
                    schema: Some(sample_products),
                })
            }
    ...
    async fn handle_event(&self, _handler_id: &str, _event: &dyn crate::event::Event) -> Result<()> {
        Ok(())
    }
}
