// src/ui/layout/sidebar.rs
use dioxus::prelude::*;
#[allow(unused_imports)]
use dioxus_router::prelude::*;
use crate::ui::{
    router::{nav,Route},
    state::{auth::use_has_permission,use_app_state},
};
                ...
                div{class:"relative flex flex-col flex-1 w-64 bg-white",
                    ...
                    div{class:"flex-1 h-0 pt-5 pb-4 overflow-y-auto",
                        ...
                        div{class:"px-2 mt-6",
                            ...
                            PluginNavigation{on_click:Some(props.on_close)}
                        }
                    }
                }
            }
        ...
        rsx!{
            if let Some(route)=&item.route{
                Link{
                    ...
                    span{
                        class:format!("text-lg {}",if collapsed{""}else{"mr-3"}),
                        "{item.icon}"
                    }
                    if!collapsed{
...
fn PluginNavigation(
    ...
    // Get real-time plugin status
    let plugin_service = crate::ui::services::plugin_service::use_plugin_service();
    let loaded_plugins = use_resource(move || {
        let service = plugin_service.clone(); // Clone before moving into async block
        async move {
    ...
    match&*loaded_plugins.read_unchecked(){
        Some(plugins)=>{
            if plugins.is_empty(){
                rsx!{
