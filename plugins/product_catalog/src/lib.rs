// plugins/product_catalog/src/lib.rs
...
impl Default for PluginConfig {
    fn default() -> Self {
        Self {
            ...
            database_url: Some("sqlite://./data/products.db".to_string()), // Provide default
            use_api: true, // Use API by default on all platforms
...
#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl ProductDataSource for MockDataSource {
    async fn get_products(&self, limit: Option<usize>, _offset: Option<usize>) -> Result<Vec<Product>> {
        let products = vec![
            Product {
                id: "prod_001".to_string(),
                name: "Sample Product 1".to_string(),
                description: "A sample product for demonstration".to_string(),
            ...
            Product {
                ...
                currency: "USD".to_string(),
                stock_quantity: 100,
                sku: "SAMPLE-002".to_string(),
                barcode: Some("1234567890124".to_string()),
                images: vec!["https://example.com/product2.jpg".to_string()],
                attributes: HashMap::new(),
                created_at: Time::now(),
                updated_at: Time::now(),
                is_active: true,
        ...
        let limited_products = if let Some(limit) = limit {
            products.into_iter().take(limit).collect()
        } else {
            products
        };
    ...
    async fn get_product(&self, id: &str) -> Result<Option<Product>> {
        let products = self.get_products(None, None).await?;
        Ok(products.into_iter().find(|p| p.id == id))
    }

    async fn search_products(&self, query: &str, limit: Option<usize>) -> Result<Vec<Product>> {
        let all_products = self.get_products(None, None).await?;
...
impl ApiDataSource {
    ...
    async fn make_request(&self, url: &str) -> Result<serde_json::Value> {
        // For now, just return mock data since external API calls might fail
        tracing::info!("Mock API request to: {}", url);
        ...
        Ok(serde_json::json!([
            {
                ...
                "images": ["https://api.example.com/product1.jpg"],
                "attributes": {},
...
impl ProductDataSource for ApiDataSource {
    async fn get_products(&self, limit: Option<usize>, offset: Option<usize>) -> Result<Vec<Product>> {
        ...
        let products: Vec<Product> = serde_json::from_value(json).map_err(|e| {
            Error::plugin("product_catalog", format!("Failed to parse products: {}", e))
        })?;
    ...
    async fn search_products(&self, query: &str, limit: Option<usize>) -> Result<Vec<Product>> {
        ...
        let json = self.make_request(&url).await?;
        let products: Vec<Product> = serde_json::from_value(json).map_err(|e| {
            Error::plugin("product_catalog", format!("Failed to parse search results: {}", e))
        })?;
    ...
    async fn get_categories(&self) -> Result<Vec<String>> {
        ...
        let categories: Vec<String> = serde_json::from_value(json).map_err(|e| {
            Error::plugin("product_catalog", format!("Failed to parse categories: {}", e))
        })?;
...
impl ProductCatalogPlugin {
    ...
    async fn initialize_data_source(&mut self, context: &PluginContext) -> Result<()> {
        // Try to load config from context
        ...
        // Always try to use a working data source
        let data_source: Arc<dyn ProductDataSource> = if self.config.use_api {
            if let Some(ref endpoint) = self.config.api_endpoint {
                tracing::info!("Using API data source: {}", endpoint);
                Arc::new(ApiDataSource::new(endpoint.clone()))
            } else {
                tracing::warn!("API endpoint not configured, falling back to mock data source");
                Arc::new(MockDataSource::new())
            }
        } else {
            // For database mode, always fall back to mock if database is not available
            tracing::info!("Database mode requested, but using mock data source for demo");
            Arc::new(MockDataSource::new())
        };
...
impl Plugin for ProductCatalogPlugin {
    ...
    async fn initialize(&mut self, context: PluginContext) -> Result<()> {
        tracing::info!("Initializing Product Catalog plugin");

        // Store context first
        self.context = Some(context.clone());

        // Initialize data source with error handling
        match self.initialize_data_source(&context).await {
            Ok(_) => {
                tracing::info!("Product Catalog plugin initialized successfully");
                Ok(())
            }
            Err(e) => {
                tracing::warn!("Data source initialization failed, using fallback: {}", e);
                // Use mock data source as fallback
                self.data_source = Some(Arc::new(MockDataSource::new()));
                tracing::info!("Product Catalog plugin initialized with mock data source");
                Ok(())
            }
        }
    }
    ...
    async fn shutdown(&mut self) -> Result<()> {
        tracing::info!("Shutting down Product Catalog plugin");
        Ok(())
    }
        ...
        match route_id {
            "list_products" => {
                if let Some(ref data_source) = self.data_source {
                    let limit = request.query_params.get("limit").and_then(|s| s.parse().ok());
                    let offset = request.query_params.get("offset").and_then(|s| s.parse().ok());
...
mod tests {
    ...
    fn test_config_defaults() {
        ...
        assert_eq!(config.use_api, true); // Changed to always use API
    ...
    async fn test_mock_data_source() {
        ...
        let products = data_source.get_products(Some(10), None).await.unwrap();
