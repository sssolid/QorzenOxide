...
impl Plugin for TestPlugin {
    fn info(&self) -> PluginInfo {
        PluginInfo {
            ...
            description: "A simple test plugin".to_string(),
    ...
    async fn initialize(&mut self, context: PluginContext) -> Result<()> {
        tracing::info!("Test plugin initializing");
        self.context = Some(context);
        tracing::info!("Test plugin initialized successfully");
        Ok(())
    }

    async fn shutdown(&mut self) -> Result<()> {
        tracing::info!("Test plugin shutting down");
        Ok(())
    }
    ...
    fn menu_items(&self) -> Vec<MenuItem> {
        vec![MenuItem {
            id: "test_menu".to_string(),
            label: "Test Plugin".to_string(),
            icon: Some("🧪".to_string()),
    ...
    fn render_component(&self, component_id: &str, _props: serde_json::Value) -> Result<dioxus::prelude::VNode> {
        ...
        match component_id {
            "test_component" => {
                let node = rsx! {
                    div { class: "test-plugin",
                        h2 { "🧪 Test Plugin" }
                        p { "This is a test plugin that works!" }
            ...
            _ => Err(Error::plugin("test_plugin", "Unknown component")),
        }
    }

    async fn handle_api_request(&self, _route_id: &str, _request: ApiRequest) -> Result<ApiResponse> {
        Err(Error::plugin("test_plugin", "No API routes"))
    }
