#![cfg(not(target_arch="wasm32"))]use std::env;use std::error::Error;use std::fs;use std::path::{Path,PathBuf};use std::process::{Command,exit};fn main(){let args:Vec<String>=env::args().collect();if args.len()<2{print_help();exit(1);}match args[1].as_str(){"build"=>{let plugin_name=args.get(2);build_plugins(plugin_name.map(|s|s.as_str()));}"clean"=>{let plugin_name=args.get(2);clean_plugins(plugin_name.map(|s|s.as_str()));}"list"=>{list_plugins();}"validate"=>{let plugin_name=args.get(2);validate_plugins(plugin_name.map(|s|s.as_str()));}"init"=>{if let Some(plugin_name)=args.get(2){init_plugin(plugin_name);}else{eprintln!("Error: Plugin name required for init command");exit(1);}}"help"|"--help"|"-h"=>{print_help();}_=>{eprintln!("Error: Unknown command '{}'",args[1]);print_help();exit(1);}}}fn print_help(){println!(r#"
...
"#);}fn get_target_dir()->PathBuf{if let Ok(env_path)=std::env::var("CARGO_TARGET_DIR"){return PathBuf::from(env_path);}let config_path=Path::new(".cargo").join("config.toml");if let Ok(contents)=std::fs::read_to_string(config_path){if let Ok(toml_val)=contents.parse::<toml::Value>(){if let Some(target_dir)=toml_val.get("build").and_then(|b|b.get("target-di__STRING_LITERAL_1__target")}fn build_plugins(plugin_name:Option<&str>){let plugins_dir=Path::new("plugins");if!plugins_dir.exists(){eprintln!("Error: plugins directory does not exist");exit(1);}match plugin_name{Some(name)=>build_single_plugin(name),None=>build_all_plugins(),}}fn build_single_plugin(plugin_name:&str){let plugin_path=Path::new("plugins").join(plugin_name);if!plugin_path.exists(){eprintln!("Error: Plugin '{}' not found",plugin_name);exit(1);}println!("Building plugin: {}",plugin_name);if let Err(e)=build_plugin_at_path(&plugin_path){eprintln!("Error building plugin '{}': {}",plugin_name,e);exit(1);}println!("Successfully built plugin: {}",plugin_name);}fn build_all_plugins(){let plugins_dir=Path::new("plugins");let mut built_count=0;let mut failed_count=0;for entry in fs::read_dir(plugins_dir).unwrap(){let entry=entry.unwrap();let plugin_path=entry.path();if plugin_path.is_dir(){let plugin_name=plugin_path.file_name().unwrap().to_string_lossy();println!("Building plugin: {}",plugin_name);match build_plugin_at_path(&plugin_path){Ok(())=>{println!("✓ Successfully built: {}",plugin_name);built_count +=1;}Err(e)=>{eprintln!("✗ Failed to build {}: {}",plugin_name,e);failed_count +=1;}}}}println!("\nBuild Summary:");println!("  Built: {}",built_count);println!("  Failed: {}",failed_count);if failed_count>0{exit(1);}}fn build_plugin_at_path(plugin_path:&Path)->Result<(),Box<dyn std::error::Error>>{let cargo_toml=plugin_path.join("Cargo.toml");if!cargo_toml.exists(){return Err("No Cargo.toml found in plugin directory".into());}let mut cmd=Command::new("cargo");cmd.args(&["build","--release"]).current_dir(plugin_path);cmd.env("CARGO_TARGET_DIR",get_target_dir());let output=cmd.output()?;if!output.status.success(){let stderr=String::from_utf8_lossy(&output.stderr);return Err(format!("Cargo build failed:\n{}",stderr).into());}copy_built_library(plugin_path)?;Ok(())}fn copy_built_library(plugin_path:&Path)->Result<(),Box<dyn std::error::Error>>{let plugin_name=plugin_path.file_name().unwrap().to_string_lossy();let target_base=get_target_dir();let target_release_dir=target_base.join("release");let(prefix,extension)=if cfg!(target_os="windows"){("","dll")}else if cfg!(target_os="macos"){("lib","dylib")}else{("lib","so")};let lib_name=format!("{}{}.{}",prefix,plugin_name,extension);let source_path=target_release_dir.join(&lib_name);let dest_path=plugin_path.join(&lib_name);if source_path.exists(){std::fs::copy(&source_path,&dest_path)?;println!("  Copied library: {}",lib_name);}else{return Err(format!("Built library not found: {}",source_path.display()).into());}Ok(())}fn clean_plugins(plugin_name:Option<&str>){match plugin_name{Some(name)=>clean_single_plugin(name),None=>clean_all_plugins(),}}fn clean_single_plugin(plugin_name:&str){let plugin_path=Path::new("plugins").join(plugin_name);if!plugin_path.exists(){eprintln!("Error: Plugin '{}' not found",plugin_name);exit(1);}clean_plugin_at_path(&plugin_path,plugin_name);}fn clean_all_plugins(){let plugins_dir=Path::new("plugins");for entry in fs::read_dir(plugins_dir).unwrap(){let entry=entry.unwrap();let plugin_path=entry.path();if plugin_path.is_dir(){let plugin_name=plugin_path.file_name().unwrap().to_string_lossy();clean_plugin_at_path(&plugin_path,&plugin_name);}}}fn clean_plugin_at_path(plugin_path:&Path,plugin_name:&str){println!("Cleaning plugin: {}",plugin_name);let target_base=get_target_dir();if target_base.exists(){if let Err(e)=fs::remove_dir_all(&target_base){eprintln!("Warning: Failed to remove target directory: {}",e);}}let extensions=["dll","dylib","so"];for ext in&extensions{let lib_patterns=[format!("lib{}.{}",plugin_name,ext),format!("{}.{}",plugin_name,ext),];for pattern in&lib_patterns{let lib_path=plugin_path.join(pattern);if lib_path.exists(){if let Err(e)=fs::remove_file(&lib_path){eprintln!("Warning: Failed to remove {}: {}",pattern,e);}else{println!("  Removed: {}",pattern);}}}}}fn list_plugins(){let plugins_dir=Path::new("plugins");if!plugins_dir.exists(){println!("No plugins directory found");return;}println!("Discovered plugins:");for entry in fs::read_dir(plugins_dir).unwrap(){let entry=entry.unwrap();let plugin_path=entry.path();if plugin_path.is_dir(){let plugin_name=plugin_path.file_name().unwrap().to_string_lossy();let manifest_path=plugin_path.join("plugin.toml");let cargo_path=plugin_path.join("Cargo.toml");print!("  {} ",plugin_name);if manifest_path.exists(){print!("✓ manifest ");}else{print!("✗ manifest ");}if cargo_path.exists(){print!("✓ cargo ");}else{print!("✗ cargo ");}let has_library=check_for_built_library(&plugin_path);if has_library{print!("✓ built");}else{print!("✗ built");}println!();}}}fn check_for_built_library(plugin_path:&Path)->bool{let plugin_name=plugin_path.file_name().unwrap().to_string_lossy();let extensions=["dll","dylib","so"];for ext in&extensions{let lib_patterns=[format!("lib{}.{}",plugin_name,ext),format!("{}.{}",plugin_name,ext),];for pattern in&lib_patterns{if plugin_path.join(pattern).exists(){return true;}}}false}fn validate_plugins(plugin_name:Option<&str>){match plugin_name{Some(name)=>validate_single_plugin(name),None=>validate_all_plugins(),}}fn validate_single_plugin(plugin_name:&str){let plugin_path=Path::new("plugins").join(plugin_name);if!plugin_path.exists(){eprintln!("Error: Plugin '{}' not found",plugin_name);exit(1);}validate_plugin_at_path(&plugin_path,plugin_name);}fn validate_all_plugins(){let plugins_dir=Path::new("plugins");for entry in fs::read_dir(plugins_dir).unwrap(){let entry=entry.unwrap();let plugin_path=entry.path();if plugin_path.is_dir(){let plugin_name=plugin_path.file_name().unwrap().to_string_lossy();validate_plugin_at_path(&plugin_path,&plugin_name);}}}fn validate_plugin_at_path(plugin_path:&Path,plugin_name:&str){println!("Validating plugin: {}",plugin_name);let mut errors=Vec::new();let mut warnings=Vec::new();let manifest_path=plugin_path.join("plugin.toml");let cargo_path=plugin_path.join("Cargo.toml");let src_path=plugin_path.join("src").join("lib.rs");if!manifest_path.exists(){errors.push("Missing plugin.toml manifest file");}if!cargo_path.exists(){errors.push("Missing Cargo.toml build file");}if!src_path.exists(){errors.push("Missing src/lib.rs source file");}let mut errors:Vec<String>=Vec::new();if manifest_path.exists(){if let Err(e)=validate_manifest(&manifest_path){errors.push(format!("Invalid manifest: {}",e));}}if!check_for_built_library(plugin_path){warnings.push("No built library found - run build command");}if errors.is_empty()&&warnings.is_empty(){println!("  ✓ Plugin is valid");}else{if!errors.is_empty(){for error in errors{println!("  ✗ Error: {}",error);}}if!warnings.is_empty(){for warning in warnings{println!("  ⚠ Warning: {}",warning);}}}}fn validate_manifest(manifest_path:&Path)->Result<(),String>{let content=fs::read_to_string(manifest_path).map_err(|e|format!("Failed to read manifest: {}",e))?;let required_fields=["id","name","version"];for field in&required_fields{if!content.contains(&format!("{} =",field)){return Err(format!("Missing required field: {}",field));}}Ok(())}fn init_plugin(plugin_name:&str){let plugin_path=Path::new("plugins").join(plugin_name);if plugin_path.exists(){eprintln!("Error: Plugin '{}' already exists",plugin_name);exit(1);}println!("Creating plugin: {}",plugin_name);fs::create_dir_all(&plugin_path).unwrap();fs::create_dir_all(plugin_path.join("src")).unwrap();let cargo_toml=format!(r#"[package]
name = "{}"
version = "0.1.0"
edition = "2021"
...
"#,plugin_name,plugin_name.replace('_'," ").replace('-'," "),plugin_name);fs::write(plugin_path.join("plugin.toml"),plugin_toml).unwrap();let lib_rs=format!(r#"use qorzen_oxide::plugin::*;
...
"#,plugin_name,plugin_name.replace('_'," ").replace('-'," "),plugin_name,plugin_name);fs::write(plugin_path.join("src").join("lib.rs"),lib_rs).unwrap();println!("✓ Plugin '{}' created successfully",plugin_name);println!("  Next steps:");println!("    1. cd plugins/{}",plugin_name);println!("    2. Implement your plugin logic in src/lib.rs");println!("    3. cargo run --bin build_plugins build {}",plugin_name);}
